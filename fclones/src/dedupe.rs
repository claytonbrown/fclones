...
use crate::metadata_preserve::{merge_metadata_from_duplicates, is_image_file as is_image_for_metadata};
use crate::path::Path;
...
impl FsCommand {
    ...
    fn symlink_with_metadata(target: &Path, link: &Path, preserve_metadata: bool, all_files: &[&Path]) -> io::Result<()> {
        #[cfg(feature = "metadata-preserve")]
        if preserve_metadata && is_image_for_metadata(&target.to_path_buf()) {
            if let Ok(merged_metadata) = merge_metadata_from_duplicates(all_files) {
                let _ = merged_metadata.apply(&target.to_path_buf());
            }
        }
        Self::symlink(target, link)
    }

    fn hardlink_with_metadata(target: &Path, link: &Path, preserve_metadata: bool, all_files: &[&Path]) -> io::Result<()> {
        #[cfg(feature = "metadata-preserve")]
        if preserve_metadata && is_image_for_metadata(&target.to_path_buf()) {
            if let Ok(merged_metadata) = merge_metadata_from_duplicates(all_files) {
                let _ = merged_metadata.apply(&target.to_path_buf());
    ...
    pub fn execute_with_metadata(
        ...
        match self {
            ...
            FsCommand::SoftLink { target, link } => {
                let _ = Self::maybe_lock(&link.path, should_lock)?;
                Self::safe_remove(&link.path, |link| Self::symlink_with_metadata(&target.path, link, preserve_metadata, all_files), log)?;
                Ok(link.metadata.len())
            }
            FsCommand::HardLink { target, link } => {
                let _ = Self::maybe_lock(&link.path, should_lock)?;
                Self::safe_remove(&link.path, |link| Self::hardlink_with_metadata(&target.path, link, preserve_metadata, all_files), log)?;
                Ok(link.metadata.len())
            }
            FsCommand::RefLink { target, link } => {
                let _ = Self::maybe_lock(&link.path, should_lock)?;
                #[cfg(feature = "metadata-preserve")]
                if preserve_metadata && is_image_for_metadata(&target.path.to_path_buf()) {
                    if let Ok(merged_metadata) = merge_metadata_from_duplicates(all_files) {
                        let _ = merged_metadata.apply(&target.path.to_path_buf());
                    }
                }
                crate::reflink::reflink(target, link, log)?;
                Ok(link.metadata.len())
...
/// Runs a deduplication script generated by [`dedupe`] with metadata preservation support.
///
...
pub fn run_script_with_metadata<I>(
    script: I, 
    should_lock: bool, 
    preserve_metadata: bool,
    log: &dyn Log
    ...
    I: IntoParallelIterator<Item = (usize, Vec<FsCommand>)>,
{
    script
        ...
        .map(|cmd| {
            // Collect all file paths for metadata merging
            cmd.execute_with_metadata(should_lock, preserve_metadata, &[], log)
        })
        .inspect(|res| {
            if let Err(e) = res {
                log.warn(e);
